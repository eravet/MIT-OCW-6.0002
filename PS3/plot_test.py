# -*- coding: utf-8 -*-
"""
Created on Tue Jun 16 13:40:54 2020
...
#         #create dictionary of tile positions and dirt amount
...
#         #tile dirt level cannot be less than 0
...
#         """
#         if math.floor(pos.get_x()) <= self.width and math.floor(pos.get_y()) <= self.height:
#             return True
#         else:
#             return False
...
#         new_position = position.get_new_position(angle, self.speed)
...
#             angle = random.random() * 360
#             self.set_robot_direction(angle)
#         elif self.room.is_position_valid(new_position):
#             self.set_robot_position(new_position)
#             self.room.clean_tile_at_position(new_position, self.capacity)
#         else:
#             angle = random.random() * 360
...
#             robots.append(robot)
#         while min_coverage > room.get_num_cleaned_tiles()/float(room.get_num_tiles()):
#             timestep += 1
#             for robot in robots:
...
def show1_plot_compare_strategies(title, x_label, y_label):
    """
    Produces a plot comparing the two robot strategies in a 20x20 room with 80%
    minimum coverage.
    """
    ...
    for num_robots in num_robot_range:
        print ("Plotting", num_robots, "robots...")
        times1.append(num_robots**2)
        times2.append(num_robots)
    print(times1)
    pylab.plot(num_robot_range, times1)
    pylab.plot(num_robot_range, times2)
    pylab.title(title)
...
show1_plot_compare_strategies('Time to clean 80% of a 20x20 room, for various numbers of robots','Number of robots','Time / steps')
