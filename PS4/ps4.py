...
class Patient(object):
    ...
    def update(self):
        ...
        for cell in surviving_bacteria:
            try:
                new_cell = cell.reproduce(self.pop_density)
                new_bacteria.append(new_cell)
...
def calc_pop_avg(populations, n):
    ...
    for trial in range(len(populations)):
        total_trial_pop += populations[trial][n]
    return total_trial_pop/len(populations)
...
# average_pop = []
# for step in range(300):
#     average_pop.append(calc_pop_avg(populations, step))
# print(populations)
# print(average_pop)
# make_one_curve_plot(range(300), average_pop, 'Timestep', 'Average Population', 'Without Antibiotic')
...
def calc_pop_std(populations, t):
    ...
    std = 0
    for trial in range(len(populations)):
        std += (populations[trial][t] - mean)**2
...
class TreatedPatient(Patient):
    ...
    def update(self):
        ...
        """
        new_bacteria = []
        for ind, cell in enumerate(self.bacteria):
            if cell.is_killed():
                self.bacteria.pop(ind)
            elif self.on_antibiotic == True:
                if not cell.get_resistant():
                    self.bacteria.pop(ind)
            else:
                self.pop_density = self.get_total_pop()/float(self.max_pop)
                try:
                    new_bacteria.append(cell.reproduce(self.pop_density))
                except NoChildException:
                    continue
...
def simulation_with_antibiotic(num_bacteria,
    ...
    for trial in range(num_trials):
        ...
        populations.append([None] * 400)
        resistant_pop.append([None] * 400)
        populations[trial][0] = num_bacteria
        resistant_pop[trial][0] = 0
...
resistant_pop_avg = []
for i in range(400):
    total_pop_avg.append(calc_pop_avg(total_pop, i))
    resistant_pop_avg.append(calc_pop_avg(resistant_pop, i))
print(len(total_pop_avg))
print(len(resistant_pop_avg))
make_two_curve_plot(range(400), total_pop_avg, resistant_pop_avg, 'Total', 'Resistant', 'Timestep', 'Average Population', 'With an Antibiotic')